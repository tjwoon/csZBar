...
implements SurfaceHolder.Callback {
	...
	private void setUpCamera() {
		...
		scannerSurface.setLayoutParams(new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT, Gravity.CENTER));
	...
	public void toggleFlash(View view) {
		...
		try {
			...
			if (flashMode.equals(Parameters.FLASH_MODE_OFF) && !(flashMode.equals(Parameters.FLASH_MODE_TORCH)) && !(flashMode.equals(Parameters.FLASH_MODE_ON))) {
	...
	private void matchSurfaceToPreviewRatio () {
		...
		if (previewRatio > surfaceRatio) {
			scannerSurface.setLayoutParams(new FrameLayout.LayoutParams(surfW, Math.round((float) surfW / previewRatio), Gravity.CENTER));
		} else if (previewRatio < surfaceRatio) {
			scannerSurface.setLayoutParams(new FrameLayout.LayoutParams(Math.round((float) surfH * previewRatio), surfH, Gravity.CENTER));
		}
	}
	...
	public Collection<ZBarcodeFormat> getFormats() {
		if (mFormats == null) return ZBarcodeFormat.ALL_FORMATS;
